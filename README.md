...
## Versioni Nel File Chart.yaml

Nel file `Chart.yaml` sono presenti tre valori con significati diversi:
...
dependencies:
  - name: ressuite
    version: 7.0.34
    repository: https://nexus.aws.res-it.com/repository/helm-release/
```

- `version` identifica la versione del chart wrapper. Puo' essere gestita
  liberamente dal cliente secondo le proprie regole di versionamento.
- `appVersion` e' un'informazione descrittiva associata al wrapper. Non
  determina quale versione di RES Suite viene installata e puo' essere gestita
  liberamente dal cliente.
- `dependencies[].version`, relativa alla dipendenza `ressuite`, identifica la
  versione effettiva del chart RES Suite che Helm deve scaricare e installare.

Per installare o aggiornare RES Suite, l'unico valore che deve essere adeguato
alla versione fornita da RES e' quindi la versione della dipendenza:

```yaml
dependencies:
  - name: ressuite
    version: <versione-res-suite>
    repository: <repository-helm>
```

Non e' necessario che `version`, `appVersion` e la versione della dipendenza
coincidano. Nel wrapper fornito inizialmente possono avere lo stesso valore, ma
questo non rappresenta un vincolo.
...
HELM_REPO_URL=https://nexus.aws.res-it.com/repository/helm-release/
