...
print_banner() {
  [ "$QUIET" -eq 0 ] || return 0
  printf '%s\n' '======================================================'
  printf '%*s%s%*s\n' 15 '' 'RES SUITE HELM BOOTSTRAP' 15 ''
  printf '%s\n' '======================================================'
}
...
finish() {
  ...
  if [ "$status" -eq 0 ]; then
    [ "$QUIET" -eq 1 ] || printf '\n'
    log_ok "Bootstrap completato in $(elapsed_since "$START_TIME")"
    return 0
  fi
...
run_step() {
  ...
  if [ "$DEBUG" -eq 1 ]; then
    ...
    if ! "$@" >"$output_file" 2>&1; then
      if [ -s "$output_file" ]; then
        log_error "Comando fallito; output diagnostico:"
        while IFS= read -r output_line || [ -n "$output_line" ]; do
          safe_output_line="$(redact_sensitive_output "$output_line")"
          [ -z "$safe_output_line" ] || log_error "  $safe_output_line"
        done < "$output_file"
      else
        log_error "Comando fallito senza fornire dettagli."
      fi
      return 1
    fi
  fi
  ...
  if [ "$elapsed" = "0s" ]; then
    ...
    log_ok "$success_message ($elapsed)"
  fi
  CURRENT_STEP=""
}
...
add_helm_repo() {
  ...
  run_step "Repository Helm" "Repository '$HELM_REPO_NAME' configurato" \
...
update_dependencies() {
  run_step "Dipendenze del chart" "Dipendenze Helm aggiornate" \
...
cleanup_helm_artifacts() {
  for output_path in "$CHART_DIR/Chart.lock"; do
    if [ -e "$output_path" ]; then
      section "Pulizia"
      rm -rf "$output_path"
      log_ok "File temporaneo rimosso: Chart.lock"
    fi
  done
}
...
copy_chart_artifact() {
  ...
  relative_target="${target_path#"$CHART_DIR"/}"
  log_ok "$label estratta: $relative_target/"
}
...
copy_values_yaml() {
  ...
  relative_target="${target_path#"$CHART_DIR"/}"
  log_ok "Template dei valori creato: $relative_target"
}
...
import_ressuite_defaults() {
  ...
  run_step "Estrazione chart RES Suite $version" "Chart $version estratto" \
